# Geological Survey

**Location:** Terrain around the village — river, hillsides, forest edges
**Present:** Whoever joins the geologist. %FOREMAN% may tag along if asked.
**Available:** Any day. Requires a player with geological knowledge.
**Cost:** 3 actions total to complete the full survey. With %FOREMAN% — 2 actions (he knows the terrain, cuts the work).

## Setup

The geologist heads out with an old military topographic map and instruments. His job is to validate the map against reality — compare marked elevations, river courses, terrain features with what's actually there. The map is outdated. The river has shifted. The terrain tells a story — soft rock, water tables, slope gradients. The river is higher than it should be. The soil near the valley floor is saturated. But proving it takes work — multiple measurement sites, samples, calculations. This isn't a quick look.

The survey route takes the geologist across the area over multiple trips. On one of the routes — ruins. Stone foundations, overgrown walls, a collapsed church. The old village. It's on the map. Nobody in the new village mentioned it.

## Opportunities

- **Foreman's knowledge** — if %FOREMAN% tags along, he points out terrain features, old drainage paths, where the water pools after rain. → Bonus info: practical flood knowledge + reduces survey cost to 2 actions
- **Spot the old village** — ruins visible through the trees on one of the survey routes. Stone, not wood — this was a proper settlement once. → Bonus info: %OLD_VILLAGE% exists, its location, and nobody in the new village brought it up
- **Spot Butcher at his house** — on the route, pass by %BUTCHER%'s place. He's there, alone, doing something. → Bonus info: where he lives, that he keeps to himself
- **Spot the drinking crew heading into the forest** — %WUJAS% and the boys walking toward the treeline with bottles. → Bonus info: the crew goes into the forest — why?

## Actions

### Survey (repeatable)
- **Requires:** Geological knowledge (player ability)
- **Cost:** 1 action per trip (3 trips needed, 2 with %FOREMAN%)
- **Outcome:** Each trip covers a section of terrain. After all trips — hard data. Water table measurements, rock composition, flood projections. The village WILL flood. This is proof, not speculation.
- **Leads to:** After completion — geological proof obtained. Unlocks "Tell Wojewoda with geological proof" in **02-arrival**. Feeds into %FOREMAN%'s engineering ending chain.

### Investigate the old village
- **Requires:** Spotted the ruins during a survey trip
- **Cost:** Free (you're already out here)
- **Outcome:** First look at %OLD_VILLAGE% — the ruins, the layout, the well visible in the centre. Something feels wrong about this place.
- **Leads to:** **12-old-village** unlocked

## Exits

- **02-arrival** — return to Wojewoda with data (if complete)
- **12-old-village** — explore the ruins now
- Any village location
