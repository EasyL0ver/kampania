...
- `gap-fill-examined` — you climbed the plug and, at the crest, saw the fill beds hard against the intact ridge with no open channel: packed clay and shattered rock, not loose rubble, and no leak path where the slide meets the ridge. The observation, not yet the conclusion. The access cost for `gap-is-blocked`. Earned at [The Ridge Gap](../locations/the-ridge-gap.md), reached from the survey routes. The seal can be judged only from the crest, which is what justifies the climb: a toe sample settles the material but not whether the plug actually holds.
