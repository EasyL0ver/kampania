...
- `streambed-parameters` — the two raw elevations that settle the streambed: the far-ridge col height and the village height. You cannot get them until you know the dam builders were here and which two points to compare (`dam-builders-surveyed-streambed`). That comes either from a geologist reading the kit's dam-survey index, where it is merely implied (a layman sees a dull list of station numbers), or from a cooperative Michał Pytlak, who remembers the dam crews and simply tells you. Then two paths, both expensive. A geologist runs a level line with the kit (Survey the streambed col), a full day of fieldwork, about 6 cards solo, cut to a floor of 3 if other PCs assist. Or anyone hunts down the dam-survey **reper** benchmarks in the field and copies the stamped figures (Read the reper benchmarks, no skill, but an open-ended search of unknown length). Raw data, not yet the conclusion.
