...
| Inspect the ditch | on-site; walk its full length (the head is concrete and misleads) | `ditch-concrete-stops-short`; head-only drainage calc gives the trap `ditch-drains-fine`; walking + recalc gives `ditch-drains-nothing`; vs the spec doc `ditch-not-built-to-spec` |
